from cb_bdti.conf.cygnus.manager import CygnusConfManager
from cb_bdti.conf.manager import ConfigManager
from cb_bdti.subscriptions.manager import SubscriptionManager
from cb_bdti.utils.helpers import Helpers
from cb_bdti.deployment.handler import DeploymentHandler
...
class BDTI(object):
	...
	def __init__(self, file_path, delete=False, deploy=True):
		...
		try:
			...
			logging.debug("Starting CB-BDTI")
			logging.debug('Reading config file from {path}'.format(path=file_path))
			ConfigManager.set_config_path(file_path)
			if not delete:
				logging.debug('Getting Orion Url')
				...
				logging.debug('Orion URL: {url}'.format(url=self.orion_url))

				logging.debug('Getting Cygnus Url')
				self.cygnus_url = Helpers.get_cygnus_url(ConfigManager.get_value(MAIN_SECTION, CYGNUS_HOST))
				logging.debug('Cygnus URL: {url}'.format(url=self.cygnus_url))
			if deploy:
				logging.debug('Instantiating deployment handler')
	...
	def create_subscriptions(self, datamodels):
		...
		:return: None
		"""
		logging.info('Creating new subscriptions')
		cont = 0
		for data_model in datamodels:
			...
			if old_subscription_id:
				logging.warning('Data Model {datamodel} is already integrated'.format(datamodel=data_model))
				modify_datamodel = Helpers.confirm_action('Do you want to modify it?')
				if modify_datamodel:
					self.modify_subscriptions([data_model])
					cont += 1
			else:
				logging.debug('Creating new subscription for {datamodel} Data Model'.format(datamodel=data_model))
				new_subscription_id = self.subscribe(data_model)
				cont += 1
				logging.info(
					'Subscription for {datamodel} Data Model created successfully'.format(datamodel=data_model))
				logging.debug('{datamodel}: {subscription_id}'.format(datamodel=data_model,
				...
				logging.debug('Subscription ID for {datamodel} saved'.format(datamodel=data_model))
	...
	def modify_subscriptions(self, data_models, force=False):
		...
		:return: None
		"""
		logging.info('Modifying subscriptions')
		cont = 0
		for data_model in data_models:
			...
			if old_subscription_id:
				...
				try:
					...
					if force:
						logging.warning(e)
						logging.info('Forcing the deletion of %s Data Model' % data_model)
					else:
						raise (e)
				logging.debug('Removed subscription for {datamodel}'.format(datamodel=data_model))
				new_subscription_id = self.subscribe(data_model)
				cont += 1
				logging.debug('New subscription for {datamodel}: {id}'.format(datamodel=data_model,
																			  id=new_subscription_id))
				ConfigManager.update_internal_datamodel(data_model, new_subscription_id, self.orion_url)
				ConfigManager.update_internal_conf_file()
				logging.debug('Subscription ID for {datamodel} saved'.format(datamodel=data_model))
				logging.info(
					'Subscription for {datamodel} Data Model modified successfully'.format(datamodel=data_model))
			else:
				logging.warning('Data Model {datamodel} is not integrated'.format(datamodel=data_model))
				integrate_datamodel = Helpers.confirm_action('Do you want to integrate it?')
	...
	def delete_subscription(data_model, force=False):
		...
		try:
			...
			logging.debug('{datamodel} Data Model subscription ID: {id}'.format(datamodel=data_model, id=subscription_id))
			...
			ConfigManager.update_internal_conf_file()
			logging.info('Subscription removed successfully')
	...
	def create_cygnus_agent(self, out_file):
		...
		:return: None
		"""
		logging.info('Creating new Cygnus Agent file')
		...
		logging.info('New Cygnus Agent file created successfully')
	...
	def create_grouping_rules(self, out_file):
		...
		:return: None
		"""
		logging.info('Creating new Grouping Rules file')
		...
		logging.info('New Grouping Rules file created successfully')
	...
	def initialize_deploy_handler(cls):
		...
		logging.debug(
			'Cygnus will be deployed {deploy_mode}'.format(deploy_mode='remotely' if cygnus_remotely else 'locally'))
	...
	def reset(cls, force):
		...
		overwrite = Helpers.confirm_action('This action will remove Orion subscriptions and '
										   'remove the configuration file.\n'
										   'Do you want to continue?')
		if overwrite:
			try:
				os.remove(PRODUCTION_INI)
				logging.info("Configuration file removed successfully")
			except Exception as e:
				logging.error(e)
	...
	def get_config_file():
		...
		try:
			if not os.path.isfile(PRODUCTION_INI):
				...
				logging.info("Configuration file created successfully: %s" % PRODUCTION_INI)
			else:
				logging.error("Configuration file already exists")
		except:
			logging.error("Cannot create configuration file: Permission denied")
	...
	def integrate(self, datamodels):
		...
		try:
			...
			logging.info('Integration process finished')
		except Exception as e:
			logging.error(e)
			logging.info('Integration process finished with errors')
	...
	def modify(self, datamodels, force=False):
		...
		:return: None
		"""
		logging.info('Starting integration modification process')
		try:
			datamodels = self.get_datamodels(datamodels)
			...
			if deploy_cygnus:
				...
				self.deployment_handler.deploy_cygnus()
			self.deployment_handler.close_handler()
			logging.info('Modify integration process finished')
		except Exception as e:
			logging.error(e)
			logging.info('Modify integration process finished with errors')
	...
	def delete(self, datamodels, deploy, force):
		...
		"""
		try:
			logging.info('Starting integration removal process')
			...
			for datamodel in datamodels2delete:
				if datamodel in integrated_datamodels:
					...
					logging.error('Data Model {datamodel} is not integrated'.format(datamodel=datamodel))

			logging.info('Cygnus will {deploy}be deployed'.format(deploy='' if deploy else 'not '))
			if deploy:
				self.create_cygnus_agent(AGENT)
				self.create_grouping_rules(GROUPING_RULES)
				self.deployment_handler.deploy_cygnus()
				self.deployment_handler.close_handler()
			logging.info('Integration removal process finished')

		except Exception as e:
			logging.error(e)
			logging.info('Integration removal process finished with errors')
