...
def delete(ctx, datamodels, deploy, force):
	""" Delete Data Models integration.
		...
		The data model to be deleted must have been previosuly integrated by using integrate command.
...
@cli.command(name="view_integrations", help_priority=4)
@click.pass_context
def view_integrations(ctx):
...
@cli.command(name="get_config", help_priority=5)
@click.pass_context
def get_config(ctx):
