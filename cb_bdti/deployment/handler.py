...
class DeploymentHandler:
	def __init__(self, remote, ip, key_path, user):
		...
		self.cygnus_ip = ip
		if remote:
			logging.info('Creating SSH session to %s' %ip)
			import paramiko
	...
	def run_cygnus(self):
		...
		command = 'sudo docker run --network=host -d -v {agent}:/opt/fiware-cygnus/docker/cygnus-ngsi/agent.conf' \
	...
	def close_handler(self):
		if self.remote:
			...
			logging.info('Closing SSH session to %s' % self.cygnus_ip)
